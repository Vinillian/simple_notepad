...
// ========== ДОБАВЛЕНО ДЛЯ ССЫЛОК ==========
let linkPreviewCache = new Map();      // Кэш метаданных ссылок
let fetchQueue = new Map();            // Очередь запросов к API
...
function loadNotes() {
    ...
    if (savedNotes) {
        ...
        // Для совместимости со старыми данными и добавления поддержки ссылок
        notes.forEach(note => {
            ...
            // ========== ДОБАВЛЕНО ДЛЯ ССЫЛОК ==========
            // Определяем тип заметки, если его нет
            if (note.type === undefined) {
                note.type = isValidURL(note.content.trim()) && note.content.trim().split('\n').length === 1 ? 'link' : 'note';
            }
            // Инициализируем metadata для ссылок
        ...
        // ========== ДОБАВЛЕНО ДЛЯ ССЫЛОК ==========
        // Медленная загрузка метаданных для новых ссылок
        setTimeout(() => {
...
function saveNotes() {
    ...
    notes.forEach(note => {
        ...
        if (!note.id) note.id = Date.now() + Math.random();
        // Убедимся, что тип и metadata сохраняются
        if (note.type === undefined) {
            note.type = isValidURL(note.content.trim()) && note.content.trim().split('\n').length === 1 ? 'link' : 'note';
        }
        if (note.type === 'link' && !note.metadata) {
...
function addNote(text, categoryId) {
    ...
    const isLink = isValidURL(trimmed) && trimmed.split('\n').length === 1;
    ...
    const newNote = {
        ...
        expanded: false,
        editMode: false,
        type: isLink ? 'link' : 'note',   // ДОБАВЛЕНО
        metadata: isLink ? null : undefined // ДОБАВЛЕНО
    };
    ...
    // ========== ДОБАВЛЕНО ДЛЯ ССЫЛОК ==========
    // Если это ссылка, загружаем метаданные асинхронно
    if (isLink) {
        setTimeout(() => fetchLinkMetadata(newNote.id, trimmed), 500);
    }
}
...
function saveEditedNote(id, newContent) {
    ...
    if (isNowLink) {
        ...
        note.type = 'note';
        delete note.metadata; // удаляем метаданные, если они были
    }
    ...
    // ========== ДОБАВЛЕНО ДЛЯ ССЫЛОК ==========
    // Если стала ссылкой и раньше не была (или изменился url), загружаем метаданные
...
// ====================
// ========== ФУНКЦИИ ДЛЯ ССЫЛОК (ИЗ notes 12.0) ==========
// ====================
...
// Асинхронная загрузка метаданных через microlink.io
async function fetchLinkMetadata(noteId, url) {
    if (linkPreviewCache.has(url)) {
        // Если метаданные уже есть в кэше, используем их
...
function updateNoteMetadata(noteId, metadata) {
    ...
    if (note) {
        ...
        saveNotes();
        displayNotes(); // перерисовываем, чтобы показать обновлённые данные
    }
}

// Загрузка метаданных для новых ссылок (при импорте или загрузке)
function loadMetadataForNewLinks() {
    const linksWithoutMetadata = notes
        .filter(item => item.type === 'link' && !item.metadata)
        .slice(0, 5); // не более 5 за раз, чтобы не перегружать API
    
    linksWithoutMetadata.forEach((item, index) => {
        setTimeout(() => {
            fetchLinkMetadata(item.id, item.content);
        }, index * 2000);
    });
}

// Рендеринг содержимого ссылки
function renderLinkContent(note) {
    ...
    if (faviconUrl) {
        html += `<img src="${faviconUrl}" alt="favicon" class="link-favicon" loading="lazy">`;
    } else {
        html += '<i class="fas fa-link link-icon-placeholder"></i>';
    }
    html += `<a href="${url}" target="_blank" rel="noopener noreferrer" class="link-url">${getShortTitle(metadata.title || domain)}</a>`;
    html += '</div>';
    html += `<div class="link-domain">${domain}</div>`;
    if (metadata.description) {
        html += `<div class="link-description">${getShortDescription(metadata)}</div>`;
    }
    html += '</div>';
    return html;
}
